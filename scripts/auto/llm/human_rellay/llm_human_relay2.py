...
class OperationSide:
    ...
    def handle_text_command(self, line):
        ...
        self.log_status(f'Command {action}: {reason} | running for 1s', command, force=True)
        ...
        with self.lock:
            self.command = self.CMD_STOP.copy()
            self.last_action = 'STOP'
            self.last_reason = f'stopped after {action.lower()}'
            self.last_raw_command = 'stop'

        self.publish_command(self.CMD_STOP)
        snapshot_path = self.save_latest_frame_snapshot(prefix=f'{action.lower()}_after_stop')

        if snapshot_path is not None:
            self.log_status(
                f'Command {action}: {reason} | stopped and saved {snapshot_path.name}',
                self.CMD_STOP,
                force=True,
            )
        else:
            self.log_status(f'Command {action}: {reason} | stopped', self.CMD_STOP, force=True)
